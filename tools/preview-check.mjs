...
// Expansion-ready stamp: move every row to In use
{
  ...
  await page.locator('#board-reset').click();
  note('reset returns the stamp to its dashed outline', await page.locator('#board-stamp').evaluate(el => el.classList.contains('is-pending') && getComputedStyle(el).borderTopStyle === 'dashed'));
}
