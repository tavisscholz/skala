...
// Readiness ladder drives the board
{
  ...
  note('hovering stage 1 swaps the board to the person-ready plan', (await page.locator('.workboard__head .eyebrow').textContent()).trim() === 'From ad hoc to person-dependent' && (await page.locator('.board-stamp__word').textContent()).trim() === 'Person ready.' && (await page.locator('.workboard__table tbody tr').first().locator('td').nth(1).textContent()).includes('best opener'));
...
note('playbook page has four articles', (await fn.locator('.fn-article').count()) === 4);
