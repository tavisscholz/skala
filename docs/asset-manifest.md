...
| Peaks | Supplied (traced) | `#mark-peaks` and `#mark-peaks-sweep` symbols; `assets/peaks.svg`, `assets/peaks-sweep.svg`; acid PNGs in `assets/brand/` | Two brush drawings supplied by Tavis on 21 September, traced with `tools/trace-mark.py` (potracer). The compact mark sits in the header and the favicon; the one with the sweep beneath goes on the hero poster and the campaign card. |
