...
| Peaks | Supplied (traced) | `#mark-peaks` and `#mark-peaks-sweep` symbols; `assets/peaks.svg`, `assets/peaks-sweep.svg`; acid PNGs in `assets/brand/` | Two brush drawings supplied by Tavis on 21 September, traced with `tools/trace-mark.py` (potracer). The one with the sweep beneath, mirrored left to right so the taller peak sits at right, is the mark in use: header, favicon, hero poster and campaign card. The compact one is kept as `#mark-peaks` for a swap. |
...
| Favicon | Created | `assets/favicon.svg` | Peaks on acid square. |
