...
| Peaks | Created | `#mark-peaks` symbol; `assets/peaks.svg` | Two filled triangles, the smaller at left behind the larger at right, separated by a thin gap, drawn as deliberate iconography (21 September). The earlier dry-brush version stays on the shelf as `#mark-peaks-brush` in the page defs and `assets/peaks-brush.svg`, generated by `tools/brush-mark.py`; point the uses at that id to bring it back. Placements: header wordmark, hero poster, campaign card. The photographs still show the crown. |
